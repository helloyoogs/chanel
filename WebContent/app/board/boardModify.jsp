...
<link rel="stylesheet" href="${pageContext.request.contextPath}/assets/css/boardModify.css" />
...
<input type="hidden" value="${sessionId}" id="admin_check"> <!-- 관리자인지 아닌지 체크 후 관리자메뉴 show -->
	...
	<!--	헤더끝-->
<form action="${pageContext.request.contextPath}/board/BoardModifyOk.bo" name="writeForm" method="post" enctype="multipart/form-data" class="table_wrap">
       <h1>글 수정</h1>
<input type="hidden" name="boardNum" value="${board.getBoardNum()}">
		<div class="tbl1">
			...
			<p>
				<input id="boardTitle" name="boardTitle" type="text"
					placeholder="제목 입력" value="${board.getBoardTitle()}">
			</p>
			<p>
				작성자 :
 ...
 <div class="upload_file1">
		...
		<div class="write_submit">
			<textarea name="boardContent" placeholder="내용 작성">${board.getBoardContent()}</textarea>
			<button type="button" value="취소" onclick="history.back()">취소</button>
			<button type="button" value="글쓰기" onclick="writeForm.submit()">글쓰기</button>
		</div> 
	</form>
<!--	푸터-->
    <footer>
