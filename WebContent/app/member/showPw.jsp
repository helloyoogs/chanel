...
<head>
   <meta charset="UTF-8">
    ...
    <link rel="stylesheet" href="${pageContext.request.contextPath}/assets/css/showId.css">
...
<input type="hidden" value="${sessionId}" id="admin_check"> <!-- 관리자인지 아닌지 체크 후 관리자메뉴 show -->
	...
	<!--	헤더끝-->
    <!--    로그인페이지-->
	<form method="post" action="${pageContext.request.contextPath}/member/MemberLogin.me" name="showPwForm">
    <div class="log-wrap">
        <!--  비밀번호찾기창-->
        <div class="log1">
				<p style="text-align: center; font-size: 2rem; margin-top: 116px;">
					<c:choose>
						<c:when test="${memberPw != null}">${memberPw}</c:when>
						<c:otherwise>비밀번호를 찾지 못했습니다.</c:otherwise>
					</c:choose>
				</p>
				<button type="button" class="btn1-1" onclick="showPwForm.submit()">로그인</button>
        </div>
    </div>
  </form>
    <!--    로그인페이지끝-->
