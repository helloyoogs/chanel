...
<link rel="stylesheet"
	...
	$("document").ready(function() {
		...
		$(".pay_check").click(function() {
			var i = $(".pay_check").index(this)
			this.form.action = "order_modify.do";
			this.form.submit();
		})	
			
			if($(".cp").val() == '미결제'){
				var i = $(".cp").index(this)
				$(".pay_check").eq(i).trigger("click");
			}
...
<input type="hidden" value="${sessionId}" id="admin_check"> <!-- 관리자인지 아닌지 체크 후 관리자메뉴 show -->
				...
				<div class="menu1">
					<div class="button2">
						...
						<form method="post">
							<ul class="amount-1">
								<li>수량</li>
								<li><input class="spinner_input cQuantity${vs.index}" type="text" value="${dto.cQuantity}" name="cQuantity" min="1" max="50" 
							...
							<table style="display:none;">
								<tr>
									...
									<td><input type="text" name="cPayment" value="${dto.cPayment}" class="cp"></td>
									<td>${dto.cDate}</td>
								</tr>
							</table>
	...
	<c:forEach items="${cart}" var="dto" varStatus="vs">
				<!--            치수선택1-->
						<form method="post">
							<ul class="amount-1" style="display:none;">
								<li><input type="button" value="수량변경" class="btn1 pay_check"></li>
							</ul>
							<table style="display:none;">
								<tr>
									<td><input class="spinner_input cQuantity${vs.index}" type="number" value="${dto.cQuantity}" name="cQuantity" min="1" max="50" 
								onchange="$('.cPriceAll${vs.index}').val( Number($('.sPrice${vs.index}').val()) * Number($('.cQuantity${vs.index}').val()) )"></td>
									<td><input type="text" name="memberId" value="${dto.memberId}"></td>
									<td><input type="text" name="cId" value="${dto.cId}"></td>
									<td><input type="text" name="cPriceAll" value="${dto.cPriceAll}" class="cPriceAll${vs.index}"></td>
									<td><input type="text" name="cPayment" value="결제"></td>
									<td>${dto.cDate}</td>
								</tr>
							</table>
						</form>
